...
<div class="container">
	<div class="row">
		<div class="col border m-3">
			<h3 class="display-6">Edit <c:out value="${meal.getName()}"/></h3>
			<form:form action="/meals/${meal.getId()}/edit" method="post" modelAttribute="meal">
				...
				<div class="form-group">
					...
					<form:select path="ingredients" class="form-control" size="7" multiple="true">
						<c:forEach var="ingredient" items="${ingredients}">
							<c:if test="${meal.getIngredients().contains(ingredient)}">
								<option value="${ingredient.getId()}" selected><c:out value="${ingredient.getName() }"></c:out></option>
							</c:if>
							<c:if test="${!meal.getIngredients().contains(ingredient)}">
				...
				<div class="form-group row">
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
					<div class="col"></div>
						<input type="submit" value="Update" class="btn btn-primary m-3 col-sm-2"/>
					<div class="col"></div>
				</div>
			</form:form>
		</div>
